...
const Searchlocation = dynamic(() => import("@/pages/searchlocation"));

const SubCity = ({ cityData, headData, filterData, combinedCities }) => {
  const router = useRouter();
  const { subcity, city } = router.query;

  // Handle loading state
  if (!router.isReady || !cityData) {
    return (
        ...
        {/* <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              image:
                "https://www.xtendedspace.com/images/offerlanding/relocation/packers-and-movers.webp",
              description:
...
/>
      ...
      <div className="max-w-[1550px] mx-auto w-full">
        ...
        <div className="">
          <Searchlocation cities={combinedCities} cityPath={city} />
        </div>
        <Footer />
      </div>
    </>
...
export async function getServerSideProps(context) {
  const { subcity, city } = context.query;

  const cities = Object.keys(citiesData || {}).length
  ...
  if (city) {
    filterData = cities?.filter((sub) => sub.parentId === city);
  }

  const combinedCities = [
    ...cities?.filter((x) => x.parentId === ""),
    ...filterData,
  ];

  if (subcity && citiesData) {
    const formattedSubcity = subcity.replace(/-/g, "");

    cityData = citiesData[formattedSubcity] || null;
    headData = cityData?.headData || null;
  }
