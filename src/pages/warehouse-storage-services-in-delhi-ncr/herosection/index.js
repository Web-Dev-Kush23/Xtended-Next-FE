...
const validateEmail = (email) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email); // Email validation regex
 ...
 const searchParams = useSearchParams();
  ...
  return (
    <>
      <Hubspot  portalId = "45810273" formId = "687023fe-6381-4006-92d6-bee28e5ce336" hubspotFormSubmit = {hubspotFormSubmit} data = {data}  utm={utm} />
...
<span className="error text-[12px] mb-2 text-red-500">{errors.checkError}</span>
              ...
              <button
                ...
                className={isBtnEnable && isChecked ? `font-bold text-[20px] bg-green-600 w-full h-auto py-10 rounded-lg animation-btn text-white`:"disable-btn text-white text-center w-full h-auto py-10 rounded-lg border-1px text-[20px] font-bold"}
                >
                 Submit
              </button>
            </form>
